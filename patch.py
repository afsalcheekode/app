...
# 1. Update _login
old_login_part = """      } else {
        // Manager login - check if user matches any school manager
        Map<String, String>? school;
        try {
          school = _allSchools.firstWhere(
        ...
        if (school != null) {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (_) => SchoolDashboardScreen(schoolName: school!['school'] ?? 'Unknown School'),
            ),
          );
        } else {
          // Fallback for simple testing if needed, or error
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Invalid credentials. Check Username or Password.')),
          );
        }
      }"""
...
new_login_part = """      } else {
        ...
        if (student != null) {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (_) => StudentBoardScreen(studentData: student!))
          );
        } else {
          // Manager login
          Map<String, String>? school;
...
text = text.replace(old_login_part, new_login_part)

# 2. Add teacherUsername to TeacherBoardScreen
old_teacher_call = """            builder: (_) => TeacherBoardScreen(
              teacherName: teacher!['name'] ?? 'Teacher',
              assignedClass: teacher['class'] ?? '',
              subjects: teacher['subjects'] ?? '',
            ),"""
new_teacher_call = """            builder: (_) => TeacherBoardScreen(
              teacherName: teacher!['name'] ?? 'Teacher',
              assignedClass: teacher['class'] ?? '',
              subjects: teacher['subjects'] ?? '',
              teacherUsername: teacher['username'] ?? '',
            ),"""
text = text.replace(old_teacher_call, new_teacher_call)

old_teacher_constructor = """class TeacherBoardScreen extends StatefulWidget {
...
new_teacher_constructor = """class TeacherBoardScreen extends StatefulWidget {
  final String teacherName;
  final String assignedClass;
  final String subjects;
...
text = text.replace(old_teacher_constructor, new_teacher_constructor)

# 3. Replace Manager's _buildMessagesTab
manager_replace_re = re.compile(r'(Widget _buildMessagesTab\(ColorScheme colorScheme\) \{)(.*?_showAddMessageDialog.*?\);[\n\s]*\})', re.DOTALL)
matches = manager_replace_re.findall(text)
...
if len(matches) == 2:
    ...
    print(f'Expected 2 matches for _buildMessagesTab, found {len(matches)}')
...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Legacy support: if 'convId' doesn't exist, we skip
      ...
      appBar: AppBar(
        title: Row(
          children: [
            ...
            Text(widget.chatName, style: const TextStyle(color: Colors.white, fontSize: 16)),
          ],
        ),
        backgroundColor: colorScheme.primary,
...
if "StudentBoardScreen" not in text:
    text += new_code
